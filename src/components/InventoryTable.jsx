...
export default function InventoryTable() {
  return (
    <>
      ...
      <ScrollArea className="max-h-[35rem] overflow-x-auto max-w-[86vw] sm:max-w-full">
        <div className="w-[100%] xl:hidden">
          <ul>
            {product_list.map((inventory) => (
              <li className="border border-gray-100 rounded-md p-5 sm:p-8 capitalize mb-3 text-lg md:text-base w-[400px] sm:w-full">
                <div className="table-item border-b border-gray-100 pb-2">
                  ...
                  <div>
                    <img
                      src={inventory.image}
                      alt={inventory.name}
                      className="max-w-[90px] md:mx-auto  md:max-h-[93px] md:max-w-[109px] w-full h-full object-cover"
                    />
                    {inventory.name}
                  </div>
                </div>
